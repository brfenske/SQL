IF EXISTS (
		SELECT *
		FROM sys.objects
		WHERE object_id = OBJECT_ID(N'dbo.Velocity')
			AND type IN (
				N'P'
				, N'PC'
				)
		)
	DROP PROCEDURE dbo.Velocity;
GO
...
BEGIN
	...
	WHERE ReferenceName = 'System.LinkTypes.Hierarchy'
		...
		select top 10 * from CurrentWorkItemView
	...
	--		--, u.Microsoft_VSTS_Scheduling_Effort AS Effort
	--		, SUM(u.Microsoft_VSTS_Scheduling_Effort) AS Effort
	--	FROM UserStories u
